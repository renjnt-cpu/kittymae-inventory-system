...
// item-monitoring.html) that pass just a query see no change at all. The POS product
// grid passes it to get a per-branch qty_available back on each result, flattened out
// of the embedded inventory row so callers don't need to know it's a to-many relation.
...
export async function listPullOuts() {
  const { data, error } = await supabase
    ...
    .select('*, products(item_name, gross_weight_g, system_selling_price, pricing_mode, current_gold_rate_per_g), branches(name)')
...
/** The one exception to "strictly HR Supervisor + Admin" above -- every employee sees
 * the birthday banner regardless of their own HR access, so this SECURITY DEFINER
 * function exposes just a name, nothing else from the 201 File (Ren, 2026-09-24). */
...
export async function upsertEmployee201File(employeeId, fields) {
  ...
  const { error } = await supabase.from('employee_201_files').upsert({
    ...
    educational_attainment: fields.educationalAttainment || null,
    store_category: fields.storeCategory || null,
